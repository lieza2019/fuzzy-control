...
sym_lookup :: Symtbl -> (Sym_category, (Integer, Syntree_node -> Bool)) -> String -> (Maybe ((Sym_attrib, (Sym_category, (Sym_tbl, Sym_tbl))), Symtbl), [Error_codes])
sym_lookup symtbl (cat, (depth, declp)) ident =
  ras_trace "in sym_lookup" (
  let ((left, stbl), last_id) = sym_categorize' symtbl cat
  in
    if (depth > 0) || (depth < 0) then
      case sym_search stbl ident of
        (Nothing, err) -> (Nothing, err)
        (Just (attr, h@(past, remains)), err) -> if declp (sym_attr_entity attr) then (Just ((attr, (cat, h)), symtbl), err)
                                                 else
                                                   let remains' = case remains of
                                                                    ...
                                                                    Scope_add (id, lv, anon_idents, Sym_add s ss) ps -> Scope_add (id, lv, anon_idents, ss) ps
                                                   in
                                                     case sym_lookup (sym_update symtbl cat ((left, remains'), last_id)) (cat, (depth - 1, declp)) ident of
                                                       (Just ((attr, (cat', (past', remains''))), _), err) | cat' == cat -> (Just ((attr, (cat', (past'', remains''))), symtbl'), err)
                                                         where
                                                           past'' = sym_combine past past'
                                                           symtbl' = sym_update symtbl cat ((left, sym_combine past'' remains''), last_id)
                                                         ...
                                                         where
                                                           ...
                                                           err' = err ++ [Internal_error errmsg]
                                                             where
                                                               errmsg = __FILE__ ++ ":" ++ (show (__LINE__ :: Int))
                                                       (Nothing, err) -> (Nothing, err)
    else (Nothing, [])  -- for the case of (depth == 0)
  )

sym_lkup_tydef_decl :: Symtbl -> (Integer, String) -> (Maybe ((Sym_attrib, (Sym_category, (Sym_tbl, Sym_tbl))), Symtbl), [Error_codes])
sym_lkup_tydef_decl symtbl (scp_depth, ident) =
  ras_trace "in sym_lkup_tydef_decl" (
  let declp a = case a of
                  Syn_tydef_decl _ _ -> True
                  _ -> False
  in
    case sym_lookup symtbl (Sym_cat_typedef, (-1, declp)) ident of
...
sym_lkup_fun_decl :: Symtbl -> (Integer, String) -> (Maybe ((Sym_attrib, (Sym_category, (Sym_tbl, Sym_tbl))), Symtbl), [Error_codes])
sym_lkup_fun_decl symtbl (scp_depth, ident) =
  ras_trace "in sym_lkup_fun_decl" (
  let declp a = case a of
                  Syn_fun_decl' _ _ _ _ -> True
                  _ -> False
  in
    case sym_lookup symtbl (Sym_cat_decl, (scp_depth, declp)) ident of
      r@(Just ((_, (_, _)), symtbl'), err) | verify symtbl symtbl' -> r
        where
          verify :: Symtbl -> Symtbl -> Bool
      ...
      (r@(Just ((_, (_, _)), symtbl')), err) -> (r, err ++ [Internal_error errmsg])
        where
          errmsg = __FILE__ ++ ":" ++ (show (__LINE__ :: Int))
      (Nothing, err) -> (case sym_lookup symtbl (Sym_cat_func, (-1, declp)) ident of
...
sym_lkup_rec_decl :: Symtbl -> (Integer, String) -> (Maybe ((Sym_attrib, (Sym_category, (Sym_tbl, Sym_tbl))), Symtbl), [Error_codes])
sym_lkup_rec_decl symtbl (scp_depth, ident) =
  ras_trace "in sym_lkup_rec_decl" (
  let declp a = case a of
                  Syn_rec_decl _ _ -> True
                  _ -> False
  in
    case sym_lookup symtbl (Sym_cat_record, (-1, declp)) ident of
      r@(Just ((_, (_, _)), symtbl'), err) | verify symtbl symtbl' -> r
        where
          verify :: Symtbl -> Symtbl -> Bool
...
sym_lkup_var_decl :: Symtbl -> (Integer, String) -> (Maybe ((Sym_attrib, (Sym_category, (Sym_tbl, Sym_tbl))), Symtbl), [Error_codes])
sym_lkup_var_decl symtbl (scp_depth, ident) =
      ...
      | ident' == ident ->
        ...
        in
          case sym_internalerr errs_reg of
            ...
            _ -> (case sym_lkup_var_decl symtbl' (1, ident) of
...
data Syntree_node =
  ...
  | Syn_arg_decl (String, (Integer, Integer)) Type
  | Syn_rec_decl String Type
  -- | Syn_var_decl String Type
  -- | Syn_var_decl (String, Integer) Type
  | Syn_var_decl (String, (Integer, Integer)) Type
                  ...
                  r <- lift (case tokens of
                               (Tk_ident arg_id):ts -> do
                                 ...
                                 return (case r_arg of
                                           Left err -> Left err
                                           Right ((Just arg_decl@(Syn_var_decl (arg_id, (-1, -1)) arg_ty), symtbl', ts'), errs) -> Right ((Just arg_decl, symtbl', ts'), errs)
        ...
        r <- lift (case fun_app of
                     Syn_expr_call fun_id app_args fun_ty -> do
                       case tokens of
                         ...
                         _ -> do
                           let (r_lk, err_lok) = sym_lkup_fun_decl symtbl (-1, fun_id)
                           case r_lk of
        ...
        Tk_L_bra:ts ->
          if comp_parsp then do
            case ts of
              ...
              _ -> do
                let lv_before = sym_crnt_level $ sym_scope_right (sym_categorize symtbl Sym_cat_decl)
                let (symtbl', err_scp) = sym_enter_scope (Just symtbl) Sym_cat_decl
                case sym_internalerr err_scp of
                  (e:_, _) -> throwE (Error_Excep Excep_assert_failed errmsg)
                    where
                      errmsg = __FILE__ ++ ":" ++ (show (__LINE__ :: Int))
                  _ -> do
                    r <- lift (do
                                  ...
                                  case r_comp of
                                    Left err_exc -> return $ Left err_exc
                                    Right ((Nothing, symtbl'', ts'), err) -> cat_err (err_scp ++ err) $ closing_comp Nothing (symtbl'', lv_before) ts'
                                         ...
                                         Right declp -> do
                                           ...
                                           case needs_delim of
                                             ...
                                             Right smclp -> do
                                               ...
                                               case r_comp' of
                                                 Left err_exc -> return $ Left err_exc
                                                 Right ((body, stbl, ts''), err') -> cat_err (err_scp ++ err'') $ closing_comp (Just body) (stbl, lv_before) ts''
                                                   where
                                                     err'' = err ++ err'
                                      )
                                      where
                  ...
                  where
                    closing_comp :: Maybe ([Syntree_node], [Syntree_node])  -> (Symtbl, Scope_Lv) -> [Tk_code]
                                 -> IO (Either Error_Excep ((([Syntree_node], [Syntree_node]), Symtbl, [Tk_code]), [Error_codes]))
                    closing_comp comp_body (symtbl, lv_before) tokens =
                      let body = case comp_body of
                                   Nothing -> ([], [])
                                   Just b -> b
                      in
                        case tokens of
                          Tk_R_bra:ts -> do
                            let (symtbl', err_scp) = sym_leave_scope symtbl Sym_cat_decl
                            case sym_internalerr err_scp of
                              (e:_, _) -> return $ Left (Error_Excep Excep_assert_failed errmsg)
                                where
                                  errmsg = __FILE__ ++ ":" ++ (show (__LINE__ :: Int))
                              _ -> if lv_before == lv_after then
                                     return $ Right ((body, symtbl', ts), err_scp)
                                   else
                                     let errmsg = __FILE__ ++ ":" ++ (show (__LINE__ :: Int))
                                     in
                                       return $ Left (Error_Excep Excep_assert_failed errmsg)
                                where
                                  lv_after = sym_crnt_level $ sym_scope_right (sym_categorize symtbl' Sym_cat_decl)
                          _ -> return $ Right ((body, symtbl, tokens), [Parse_error errmsg_R_bra])
          else
                                      ...
                                      (Tk_ident ident):ts' -> do
                                        ...
                                        case r_v of
                                          ...
                                          Right ((Just (var_decl@(Syn_var_decl (var_id, _) var_ty)), symtbl', tokens'), errs) -> do
                                            ...
                                            case r_cur of
                                              Left [Internal_error errmsg] -> return $ Left (Error_Excep Excep_assert_failed errmsg)
                                              Right (var_decl', symtbl'_cur) -> do
                                                let (r_redef_chk, err_redef_chk) = sym_lkup_var_decl symtbl'_cur (1, var_id)
                                                case sym_internalerr err_redef_chk of
                                                  (e:_, _) -> return $ Left (Error_Excep Excep_assert_failed errmsg)
                                                    where
                                                      errmsg = __FILE__ ++ ":" ++ (show (__LINE__ :: Int))
                                                  _ -> (case r_redef_chk of
                                                          Nothing -> 
                                                            (case sym_regist_var_decl symtbl'_cur (var_id, var_decl') of
                                                               ((symtbl'', Nothing), err_reg) -> return $ Left (Error_Excep Excep_assert_failed errmsg)
                                                                 where
                                                                   errmsg = __FILE__ ++ ":" ++ (show (__LINE__ :: Int))
                                                               ((symtbl'', Just (((key, var_id'), a@(Sym_attrib {sym_attr_entity = var_decl''@(Syn_var_decl (var_id'', key') var_ty')})), h)),
                                                                err_reg) | ((var_id' == var_id) && (var_id'' == var_id')) && (key' == key) ->
                                                          ...
                                                          (Just (_, symtbl'')) -> return $ Right ((Just var_decl, symtbl'', tokens'), errs' ++ [Symbol_redefinition errmsg])
                                                            where
                                                              errmsg = "Redefinition of " ++ var_id ++ " in variables declaration."
                                                              errs' = errs ++ err_redef_chk
                                                       )
        ...
        (Tk_ident ident):ts -> do
          r <- lift (do 
                        case ts of
                          ...
                          _ -> do
                            let var = Syn_var ident Ty_abs
                            case sym_lkup_var_decl symtbl (-1, ident) of
                                                      ...
                                                      Just (((key, ident''), Sym_attrib {sym_attr_entity = a}), h_reg@(Sym_cat_decl, _))
                                                        | ident'' == ident -> (case a of
                                                                                 Syn_var_decl (v_id, key') v_ty''
                                                                                   | (v_id == ident) && (v_ty'' == v_ty') -> cat_err err'' (runExceptT $ cont_par symtbl'' var' ts)
                                                                                   where
                                                                                     err'' = err' ++ err_reg
                                                                                 _ -> return $ Left (Error_Excep Excep_assert_failed errmsg)
                                                                                   where
...
ty_chk_var_decl symtbl (v_id, v_ty) =
  case sym_lkup_var_decl symtbl (-1, v_id) of
...
ty_inf symtbl expr =
  case expr of
    ...
    Syn_expr_call fun_id args ty -> do
      case sym_lkup_fun_decl symtbl (-1, fun_id) of
